...
async function Header() {
  ...
  return (
    <div className="relative z-10">      <div
      className="flex flex-col sm:flex-row items-center sm:justify-between justify-center gap-4 sm:gap-0
        bg-[#0a0a0f]/80 backdrop-blur-xl p-3 sm:p-4 md:p-5 mb-3 sm:mb-4 rounded-lg"
    >
      ...
      </div>        <div className="flex flex-wrap items-center justify-center sm:justify-start gap-2 sm:gap-3">
        <div className="flex items-center gap-2">
          <ThemeSelector />
          <LanguageSelector hasAccess={Boolean(convexUser?.isPro)} />
        </div>

        {/* To check if user is subscribed pro or not  */}
        {!convexUser?.isPro && (
          <Link
            href="/pricing"
            className="flex items-center gap-1.5 px-2.5 sm:px-3 py-1.5 rounded-lg border border-amber-500/20 hover:border-amber-500/40 bg-gradient-to-r from-amber-500/10 
                to-orange-500/10 hover:from-amber-500/20 hover:to-orange-500/20 
                transition-all duration-300"
          >
            <Sparkles className="w-3.5 h-3.5 text-amber-400 hover:text-amber-300" />
