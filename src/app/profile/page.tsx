  ...
  return (
    <div className="min-h-screen bg-[#0a0a0f]">
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-4 py-6 sm:py-12">
        ...
        {/* Main content */}
        <div
          className="bg-linear-to-br from-[#12121a] to-[#1a1a2e] rounded-xl sm:rounded-3xl shadow-2xl 
            ...
            <div className="flex space-x-1 p-2 sm:p-4">
              {TABS.map((tab) => (
                <button
                  ...
                  className={`group flex items-center gap-1.5 sm:gap-2 px-3 sm:px-6 py-1.5 sm:py-2.5 rounded-lg transition-all duration-200 relative overflow-hidden ${activeTab === tab.id ? "text-blue-400" : "text-gray-400 hover:text-gray-300"
                    }`}
                >
                  {activeTab === tab.id && (
                    <motion.div
              ...
              {activeTab === "executions" && (
                <div className="space-y-4 sm:space-y-6">
                  {executions?.map((execution) => (
                    <div
                      key={execution._id}
                      className="group rounded-lg sm:rounded-xl overflow-hidden transition-all duration-300 hover:border-blue-500/50 hover:shadow-md hover:shadow-blue-500/50"
                    >
                      <div className="flex items-center justify-between p-3 sm:p-4 bg-black/30 border border-gray-800/50 rounded-t-lg sm:rounded-t-xl">
                      ...
                      <div className="p-3 sm:p-4 bg-black/20 rounded-b-lg sm:rounded-b-xl border border-t-0 border-gray-800/50">
                        <CodeBlock code={execution.code} language={execution.language} />

                        {(execution.output || execution.error) && (
                          <div className="mt-3 sm:mt-4 p-2.5 sm:p-4 rounded-lg bg-black/40">
                            <h4 className="text-xs sm:text-sm font-medium text-gray-400 mb-1.5 sm:mb-2">Output</h4>
                            <pre
                              className={`text-xs sm:text-sm ${execution.error ? "text-red-400" : "text-green-400"
                                }`}
                            >
                  ...
                  {isLoadingExecutions ? (
                    <div className="text-center py-6 sm:py-12">
                      ...
                      <h3 className="text-base sm:text-lg font-medium text-gray-400 mb-1 sm:mb-2">
                    ...
                    executions.length === 0 && (
                      <div className="text-center py-6 sm:py-12">
                        ...
                        <p className="text-xs sm:text-sm text-gray-500">Start coding to see your execution history!</p>
                      </div>
                    )
                  )}
                  ...
                  {executionStatus === "CanLoadMore" && (
                    <div className="flex justify-center mt-4 sm:mt-8">
                      <button
                        ...
                        className="px-4 sm:px-6 py-2 sm:py-3 bg-blue-500/10 hover:bg-blue-500/20 text-blue-400 rounded-lg flex items-center gap-1.5 sm:gap-2 
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-6">
                  {starredSnippets?.map((snippet) => (
                    <div key={snippet._id} className="group relative">
                      <Link href={`/snippets/${snippet._id}`}>
                        <div
                          className="bg-black/20 rounded-lg sm:rounded-xl border border-gray-800/50 hover:border-gray-700/50 
                        ...
                        >
                          <div className="p-3 sm:p-6">
                            <div className="flex items-center justify-between mb-3 sm:mb-4">
                              <div className="flex items-center gap-2 sm:gap-3">
                                ...
                                <span className="px-2 sm:px-3 py-0.5 sm:py-1 bg-blue-500/10 text-blue-400 rounded-lg text-xs sm:text-sm">
                                  {snippet.language}
                                </span>
                              </div>
                              <div
                          ...
                          <div className="px-3 sm:px-6 pb-3 sm:pb-6">
                            <div className="bg-black/30 rounded-lg p-2.5 sm:p-4 overflow-hidden">
                              <pre className="text-xs sm:text-sm text-gray-300 font-mono line-clamp-3">
                                {snippet.code}
                              </pre>
                            </div>
                          </div>
                  ...
                  {(!starredSnippets || starredSnippets.length === 0) && (
                    <div className="col-span-full text-center py-6 sm:py-12">
                      <Star className="w-8 h-8 sm:w-12 sm:h-12 text-gray-600 mx-auto mb-3 sm:mb-4" />
                      <h3 className="text-base sm:text-lg font-medium text-gray-400 mb-1 sm:mb-2">
